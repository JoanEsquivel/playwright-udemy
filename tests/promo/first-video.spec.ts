...
test.describe('Good stuff from Playwright #1', () => {
    test('#1- Info annotation', async ({ page }) => {
        ...
        await expect(page.locator('.view label')).toHaveText(['good stuff #2'])
    })
    ...
    test('#3- Debug options', async ({ page }) => {
        ...
        // Make sure the list only has one todo item.
        await expect(page.locator('.view label')).toHaveText(['good stuff #1'])
    })
    ...
    test.only('#4- Soft Assertions', async ({ page }) => {
        ...
        await expect(page.locator('.view label')).toHaveText(['good stuff #3'])
