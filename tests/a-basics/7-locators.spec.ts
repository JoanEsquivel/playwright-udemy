...
test.describe('Locators', () => {
    ...
    test('Locators Examples', async ({ page }) => {
        await page.getByPlaceholder('Full Name').fill('Joan Esquivel')
        await page.locator('[type="email"]').fill('joan@test.com')
        await page
            .locator('//textarea[@placeholder="Current Address"]')
            .fill('my current address')
        await page.locator('#permanentAddress').fill('my permanent address')
